"""Run SCRFD detection, alignment, and brightness analysis on an image."""
...
def main() -> None:
    """Run detection, alignment, and brightness analysis on every detected face."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Run SCRFD detection, alignment, and brightness analysis."
    )
    ...
    for index, face in enumerate(faces, start=1):
        ...
        result = brightness_analyzer.evaluate(face.alignment.aligned_image)

        print(f"Face Index: {index}")
        print(f"Mean Brightness: {result.mean_brightness:.2f}")
        print(f"Brightness Score: {result.score:.4f}")
        print()
